    ...
    return this.http.get('data/athlete_events.csv', { responseType: 'text' })
      .pipe(map(
        data => {
          ...
          for (let index = 1; index < papaResult.length - 1; index++) {
            let row = papaResult[index];
            let newEntry = new AthleteEntry(parseInt(row[0], 10), row[1], row[2], parseInt(row[3], 10), parseInt(row[4], 10), parseInt(row[5], 10), row[6], row[7], row[8], parseInt(row[9], 10), row[10], row[11], row[12], row[13], row[14], row[15]);
            // If age was 20 at games of 1990: he is now 2022 - 1990 + 20 years old
            let athleteBirthYear = newEntry.year - newEntry.age
            let athlete = new Athlete(newEntry.id, newEntry.name, newEntry.sex, athleteBirthYear, newEntry.height, newEntry.weight, newEntry.noc, newEntry.photoUrl, newEntry.team);
